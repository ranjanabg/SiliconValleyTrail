...
News is fetched at most once every 3 days (2-day cooldown after each trigger) and only fires 25% of the time when no API key is set (mock mode). Real mode scans headline keywords and maps them to effects:
...
```
fund          — starts $50,000, min 0 (no negative debt)
morale        — 0–100
energy        — 0–100
progress      — 0–100 (% of journey complete)
connections   — 0–100
hype          — 0–100
techDebt      — 0–100
day           — starts 1, increments each day
lastRestDay   — tracks cooldown (-10 initial = "never")
lastInvestorMeetingDay — tracks cooldown (-10 initial)
nextMilestoneIndex — which milestone to check next
gameOver      — boolean flag
```

All delta methods clamp at boundaries — you can't go below 0 or above 100 on any stat. Fund is floored at 0 (no negative balance).
